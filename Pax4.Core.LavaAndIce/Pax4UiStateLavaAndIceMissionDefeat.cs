...
{
    [DataContract]
    [KnownType(typeof(Pax4UiStateLavaAndIceDefeat))]
    public class Pax4UiStateLavaAndIceDefeat : Pax4UiState
    {
        public Pax4UiStateLavaAndIceDefeat(String p_name, Pax4Ui p_ui)
            : base(p_name, p_ui)
        {
            float duration = 0.5f;
            //float delay = 0.0f;
            ...
            AddChild(sprite);
            textureName = "Sprite/lavaandiceInGameMenuTextDefeat";
            texture = Pax4Texture2D._current.Get(textureName);
            ((Pax4SpriteTexture)sprite).SetTexture(texture);
            sprite.SetPosition(position);
            colorModifierEnter.AddChild(sprite);
        ...
        {
            Pax4Sound._current.PlayStateSong("Sound/lavaandiceGameOver");
